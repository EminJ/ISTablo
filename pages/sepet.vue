...
const removeitem = (async (sepet, urun) => {
  ...
  if(req.value==null){
    const storage = localStorage.getItem('_basket')
    let f5=-1
    for (let i = 0; i < storage.split(':').length; i++) {
      const element = storage.split(':')[i].split(',');
      if(sepet[1].settings.split(',')[0]==element[0] && sepet[1].settings.split(',')[1]==element[1] && sepet[1].settings.split(',')[2]==element[2]) {
        f5=i
        break
      }
    }
    ...
    if(f5!=-1){
      for (let i = 0; i < storage.split(':').length; i++) {
       if(i!=f5) data.push(':'+storage.split(':')[i])
      }
    }
    localStorage.setItem('_basket',data)
  }
}
)
const _basket = ref([])
if (!req.value) {
  try {
    ...
    const items = await fetch(urlbase + '/api/store/frame/item/', options)
      .then(response => response.json())
      .then(response => response)
      .catch(err => err);
    for (let i = 0; i < storage.split(':').length; i++) {
      const element = storage.split(':')[i];
      for (let j = 0; j < items.length; j++) {
        const item = items[j];
        if(element.split(',')[0]==item.tabloid){
          toplam.value += item.price
          _basket.value.push([{ item: item }, { settings: element }])
        }
      }
    }
  } catch (error) { }
    ...
    <div class="w-full h-screen bg-gray-background block text-white">
      <div class="w-full h-full pt-32 flex flex-row flex-nowrap justify-center">
        <div class="w-2/4 h-auto mt-5">
          <h1 v-if="toplam <= 19" class="font-bold text-gray-200 text-2xl text-center my-6">Ürün Bulunamadı ! <a
              href="/urunler" class="text-orange-main">Göz At <i class="bx bx-link"></i> </a></h1>
          <!-- Sepet Ürün Elementi -->
          <div v-if="req" v-for="sepet in res" class="w-full h-20 bg-gray-200 rounded my-3 flex flex-nowrap"
            :id="sepet[0].item.tabloid + sepet[1].settings.size + sepet[1].settings.color + urun++">
            <!-- Sepet Yan 1. Kısım -->
            <div class="w-3/4 h-20 flex flex-nowrap text-black-main items-center">
              ...
              <div class="flex flex-col">
                <p class="w-full h-6 overflow-hidden text-black-main"><a :href="'/urunler/id/' + sepet[0].item.tabloid">{{
                  sepet[0].item.title }}</a> <span class="text-gray-primary text-xs overflow-hidden"> {{
    sepet[0].item.explanation }} </span></p>
                <div class="text-xs text-gray-primary flex">Boyut/Ebat {{ sepet[1].settings.size }}, Tablo Rengi <div
                    class="w-4 h-4 rounded-full mx-2" :style="{ backgroundColor: sepet[1].settings.color }"></div>
                </div>
            ...
            <div class="w-1/4 h-auto flex flex-nowrap items-center justify-between pr-5">
              ...
              <a href="/" onclick="return false" @click="removeitem(sepet, urun)"
                class="bx bx-trash text-black-main text-base"></a>
            </div>
          </div>
          <div v-if="_basket" v-for="sepet in _basket" class="w-full h-20 bg-gray-200 rounded my-3 flex flex-nowrap"
            :id="sepet[0].item.tabloid + sepet[1].settings.size + sepet[1].settings.color + urun++">
            <!-- Sepet Yan 1. Kısım -->
            <div class="w-3/4 h-20 flex flex-nowrap text-black-main items-center">
              <img :src="urlbase + '/images/' + sepet[0].item.images[0].split(',')[0]" class="h-full p-1 rounded-lg w-20">
    ...
    sepet[0].item.explanation }} </span></p>
          ...
          <div v-if="toplam > 19" class="w-full h-14 mb-3 flex flex-nowrap justify-between items-center">
            <div class="flex flex-nowrap items-center">
              <div v-if="toplam < 100" class="py-1 px-2 bg-yellow-700 text-gray-200 text-xs font-bold rounded-full mr-2">
                Kargo Toplam {{ convertcurr(19) }}</div>
              <div v-show="toplam>=100" class="py-1 px-2 bg-yellow-700 text-gray-200 text-xs font-bold rounded-full mr-2">
                <del>Kargo Toplam {{ convertcurr(19) }}</del></div>
            <p class="text-yellow-500 font-bold">{{convertcurr(toplam)}}</p>
          </div>
          <a href="/sepetonay"
            class="text-xl font-serif mx-6 border border-orange-main px-5 py-2 text-white-main rounded hover:bg-orange-main hover:text-black-main transition-all">Sepeti
            Onayla</a>
        </div>
      </div>
    </div>
